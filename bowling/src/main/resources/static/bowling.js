function pinButtonList(num) {
    $('#buttonList').empty();
    for (let i = 0; i < (num + 1); i++) {
        $('#buttonList').append('<button type="button" class="btn btn-success" onClick="pinCheck(' + i + ');">' + i + '개선택</button>');
    }
}
...
$(document).on('click', '#addPlayer', function() {
    ...
    console.log('buttonList 추가 전');
    for (let i = 0; i <= 10; i++) {
        $('#buttonList').append('<button type="button" class="btn btn-success" onClick="pinCheck(' + i + ');">' + i + '개선택</button>');
        console.log('buttonList 추가');
    }
});
